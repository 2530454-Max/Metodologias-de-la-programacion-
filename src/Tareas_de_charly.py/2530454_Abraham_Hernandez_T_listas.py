...
Abraham Maximiliano Hernandez Sanchez
2530454
IM 1-3
------------------------------------------------------------
RESUMEN EJECUTIVO
...
This document presents six problems designed to practice Python collections: lists, tuples, and dictionaries.
A list is an ordered and mutable collection, useful when items need to be added, removed, or modified.
A tuple is ordered but immutable, making it ideal for fixed records such as coordinates or constant datasets.
A dictionary stores key-value pairs, allowing fast lookups using a descriptive key.
Each problem includes input/output design, validation rules, and test cases, demonstrating practical 
applications such as shopping lists, product catalogs, grade management, word frequency counting, and a simple contact book.
The document highlights how collections help organize, process, and retrieve data efficiently in real-world programs.
...
Lists are ideal when data needs to be modified 
frequently, since they allow elements to be added, removed, and updated with flexibility.
Tuples are useful for storing fixed data 
that should not change, such as coordinates or configuration values.
Dictionaries make lookups easy because each value is 
accessed through a specific key, avoiding the need to search through the entire structure.
When combined, common patterns emerge—such as dictionaries that store lists—which allow data 
to be grouped while still enabling fast access through keys.
------------------------------------------------------------
Referencias
------------------------------------------------------------
