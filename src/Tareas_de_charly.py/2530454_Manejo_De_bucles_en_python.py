...
"""
This document explains that a **for loop** allows actions to be repeated by 
iterating over collections, ranges, or sequences with a known number of iterations.
A **while loop** repeats instructions as long as a logical condition 
remains true, making it more suitable when the number of repetitions is unknown.
A **counter** increases its value to keep track of iterations, 
while an **accumulator** adds values to produce partial or final totals.
It is crucial to define the **exit condition** correctly to avoid infinite 
loops that could freeze the program or consume resources.
The document will cover the description of each problem, the design of inputs
and outputs, required validations, and the appropriate use of for/while loops in traversals, 
menus, and repeated input reading.
